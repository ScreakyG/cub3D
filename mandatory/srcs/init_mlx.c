...
/*   Updated: 2024/04/08 05:10:57 by fgonzale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_mlx(t_data *data)
{
	...
	{
		printf(RED"Error with res, max res is %d * %d\n"RESET, size_x, size_y);
		ft_exit_error("", 1, data);
	}
	if (WIN_WIDTH <= 0 || WIN_HEIGHT <= 0)
	{
		printf(RED"Error with resolution, it should be superior to 0\n"RESET);
		ft_exit_error("", 1, data);
	}
	data->mlx.mlx_win = mlx_new_window(data->mlx.mlx_ptr, WIN_WIDTH,
			WIN_HEIGHT, "cub3D");
	if (!data->mlx.mlx_win)
