...
/*   Updated: 2024/03/14 14:57:04 by fgonzale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void check_floor_ceiling_colors(char *str, t_data *data)
{
	...
	tab = ft_split(str, ',');
	if (!tab)
		ft_exit_error("Malloc error textures", 1, data);
	if (tab_len(tab) != 3)
		ft_exit_error("Error with textures colors", 1, data);
	while (tab[i])
	{
		if (only_digit(tab[i]) == false)
		{
			free_strs(tab);
			ft_exit_error("Please use ints between 0 and 255 for colors", 1, data);
		}
		nb = ft_atoi(tab[i]);
		if (nb < 0 || nb > 255)
		{
